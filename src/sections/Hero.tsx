...
export const HeroSection = () => {
  ...
  const { scrollYProgress: cubeScroll } = useScroll({
    target: cubeRef,
    offset: ["start end", "end start"],
  });
  const cubeRotate = useTransform(cubeScroll, [0, 1], [100, -45]);
  const cuboidRef = useRef(null);
  const { scrollYProgress: cuboidScroll } = useScroll({
    target: cuboidRef,
    offset: ["start end", "end start"],
  });
  const cuboidRotate = useTransform(cuboidScroll, [0, 1], [20, -20]);
  const torusRef = useRef(null);
  const { scrollYProgress: torusScroll } = useScroll({
    target: torusRef,
    offset: ["start end", "end start"],
  });
  const torusRotate = useTransform(torusScroll, [0, 1], [20, -20]);
  return (
    <section className="py-24 md:py-52 overflow-x-clip">
      <div className="container">
        ...
        <div className="flex justify-center mt-10">
          <CutCornerButton className="" children="Get Started" />
        </div>
